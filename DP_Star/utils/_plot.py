...
if __name__ == '__main__':
    ...
    point_list = []
    for file in tqdm(file_list):
        with open(f'../data/{USE_DATA}/SD/sd_final_epsilon_1.0/' + file, 'r', encoding='utf-8') as traj_file: # Før f'../data/{USE_DATA}/SD/sd_final_epsilon_0.1/' Efter f'../data/{USE_DATA}/Trajectories/'
            for i in traj_file.readlines():
                point_list.append(list(map(lambda x: float(x.strip()), i.split(','))))
