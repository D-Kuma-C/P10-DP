...
def generate_adaptive_grid(mdl_trajs_dir, top_grid_path, grid_trajs_path,
    ...
    p = utils.ProgressBar(len(total_trajs), 'Mapping Grid Tracks')
    mapped_trajs = []
    ...
    if is_plot:
        ...
        p = utils.ProgressBar(len(total_trajs), 'Draw grid trajectory diagram')
        for i in range(len(total_trajs)):
            p.update(i)
...
def generate_sd_grid_mapping_traj(sd_path, grid_block_gps_range_path, sd_final_path,
    ...
    # write to files
    fcount = 0
    p = utils.ProgressBar(len(reverse_mapped_trajs), 'Generate desensitized dataset')
